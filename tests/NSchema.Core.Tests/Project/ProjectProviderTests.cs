...
public sealed class ProjectProviderTests : IDisposable
{
    ...
    public async Task GetProject_FiltersSchemaByScope()
    {
        ...
        var project = (await sut.GetProject(PlanningScope.Of(new SqlIdentifier("app")), TestContext.Current.CancellationToken)).Value!;
    ...
    public async Task GetProject_TemplateInstancesRespectScopeFilter()
    {
        ...
        var project = (await sut.GetProject(PlanningScope.Of(new SqlIdentifier("billing")), TestContext.Current.CancellationToken)).Value!;
