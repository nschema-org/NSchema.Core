...
public sealed class PlanEndToEndTests : IDisposable
{
    ...
    public async Task Plan_ReportsStructuredDiffBetweenCurrentAndDesired()
    {
        ...
        using var app = NewBuilder(current).AddSqlSchemas(Path.GetDirectoryName(desired)!, Path.GetFileName(desired)).Build();
    ...
    public async Task Plan_WithNoChanges_ReportsEmptyDiff()
    {
        ...
        using var app = NewBuilder(schema).AddSqlSchemas(Path.GetDirectoryName(desired)!, Path.GetFileName(desired)).Build();
