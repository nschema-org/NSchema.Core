...
Because they're just .NET objects, the schema can be constructed in any way you like. The usual source is a SQL DDL file (see [Defining schemas](schemas.md)), but you could just as easily read from a JSON file, generate it from code, or even construct it from the database itself.
