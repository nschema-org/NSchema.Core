...
| `ISqlGenerator`                    | Generate the SQL for a migration plan, keyed by `Dialect`. Add support for another database. | `AddSqlGenerator<T>()` (or via a provider package, e.g. `UsePostgres(...)`); select with `WithDialect(...)` |
| `ISchemaDocumentSerializer`        | Read/write a desired-schema file format (JSON built-in), keyed by `Format`.                  | `AddSchemaSerializer<T>()` / `AddSchemaSerializer(instance)`                                                |
...
| `IMigrationReporter`     | Customize run output. Register several and select one with `WithOutputFormat(...)`.          | `AddReporter<T>()` / `AddReporter(instance)`                     |
...
A few seams let you register several implementations and pick one per run by a string key:

| Seam                        | Key       | Registered via             | Selected by                                                  |
|-----------------------------|-----------|----------------------------|--------------------------------------------------------------|
| `IMigrationReporter`        | `Format`  | `AddReporter<T>()`         | `WithOutputFormat(...)` / `MigrationRunOptions.OutputFormat` |
| `ISqlGenerator`             | `Dialect` | `AddSqlGenerator<T>()`     | `WithDialect(...)` / `MigrationRunOptions.Dialect`           |
| `ISchemaDocumentSerializer` | `Format`  | `AddSchemaSerializer<T>()` | the consumer (e.g. a file extension or CLI flag)             |
