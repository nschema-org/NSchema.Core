...
| `IMigrationReporter`     | Customize run output. Register several and select one with `WithOutputFormat(...)`.          | `AddReporter<T>(format)` / `AddReporter(instance)` (first-wins); `UseReporter<T>(format)` / `UseReporter(instance)` to replace |
