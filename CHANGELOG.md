...
- The NSchema **SQL DSL**, now the primary way to declare a desired schema. `DdlSchemaSerializer` (format key `sql`) parses declarative `CREATE SCHEMA` / `CREATE TABLE` (with inline, always-named constraints and inline indexes), `GRANT`, and the view/enum/sequence/function/procedure statements above from `.sql` files — it describes desired state, never `ALTER` steps, and is dialect-agnostic (canonical type names mapped to a dialect by `ISqlGenerator`). Doc-comments (`--- …`) become catalog comments, `RENAMED FROM` drives rename detection, and `CREATE PARTIAL SCHEMA` leaves undeclared tables alone. Register files with `AddSqlSchema(path)` / `AddSqlSchemasFromGlob(pattern)` / `AddSqlSchemasFromDirectory(dir)` (mirroring the JSON helpers). Columns may reference schema-qualified user-defined types (e.g. an enum as `app.status`). See `docs/dsl-grammar.md` for the full grammar.
- Config-in-SQL. DSL files may carry top-level configuration blocks: `NSCHEMA ( … )`, `BACKEND file ( … )`, `PROVIDER postgres ( … )` declaring orchestration metadata (dialect, state backend, live provider) alongside the schema, in SQL-statement form (mirroring Postgres `WITH (option = value, …)`). The core captures them into a generic `ConfigBlock` / `ConfigValue` model (`NSchema.Configuration`) but never interprets them: `DslConfigReader.Read(...)` surfaces them for a front-end, while the schema path (`DdlSchemaSerializer.Read`) ignores them. Interpretation — precedence, mapping a block to builder registration, provider dispatch, secrets-from-env — is a front-end concern.
...
- **Breaking:** The fluent `AbstractSchemaProvider` schema-builder API (the `NSchema.Schema.Fluent` namespace — `Schema()` / `Table()` / `Column()` and friends) has been removed. Declare schemas in the SQL DSL (`.sql`, the primary format) or JSON instead; to build a schema in code, implement `ISchemaProvider` directly and register it with `AddSchema<T>()`.
