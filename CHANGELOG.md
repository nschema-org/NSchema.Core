...
- Glob-based deployment scripts. `AddSqlScripts(type, baseDirectory, pattern)` and `AddSqlScripts(type, baseDirectory, Matcher)` register pre/post-deployment scripts from the SQL files a glob selects, mirroring `AddSqlSchemas`: a single provider is registered and the glob is evaluated when scripts are read, so the script set reflects the filesystem at plan/apply time, and the `Matcher` overload allows include/exclude control. A wildcard-free pattern names a single file. Unlike the desired schema, a pattern matching no files yields no scripts (rather than throwing), since having no pre/post-deployment scripts is a valid, common state.
- Inline deployment scripts in the DDL. Deployment scripts can be declared directly in `.sql` files with `PRE DEPLOYMENT '<name>' AS $$ … $$;` / `POST DEPLOYMENT '<name>' AS $$ … $$;` — `AS` introduces the body exactly as for a view, and the body is a dollar-quoted, opaque SQL block (so it may contain its own `;`), run verbatim before/after the migration. An optional `( run_outside_transaction = true )` clause marks statements the database forbids inside a transaction (e.g. `CREATE INDEX CONCURRENTLY`). `DdlReader.Instance.Read(source)` now returns these on `DdlDocument.Scripts` (alongside `Schema` and `Config`). The body is captured by a raw lexer read (`ReadDollarQuotedBody`, the dollar-quoted counterpart of the existing parenthesized-expression capture), so no new token kind was needed. `AddSqlSchemas(...)` now registers a single provider that serves both the desired schema and these scripts from the same files, parsing each file once. Scripts are plan orchestration, not schema, so they are never written back by `DdlWriter`.
...
- **Breaking:** `FileSchemaProvider` has been removed. It existed only to share file-loading across the JSON and DDL providers; with one format, that logic now lives in the internal `DdlSchemaProvider`, registered once per `AddSqlSchemas` call and evaluated at read time. The `AddFileSchemasFromGlob` / `AddFileSchemasFromDirectory` primitives, the single-file `AddSqlSchema(path)` helper, and the `AddSqlSchemasFromDirectory(dir)` helper are gone too — `AddSqlSchemas(baseDirectory, pattern)` is the single registration entry point (the pattern defaults to `**/*.sql`; a wildcard-free pattern loads one file), or implement `ISchemaProvider` directly.
- **Breaking:** The single-file `AddScriptFromFile(type, path, name)` helper and the `FileScriptProvider` it registered have been removed in favour of the glob-based `AddSqlScripts(...)` above, mirroring the schema side — a wildcard-free pattern names a single file, so the dedicated single-file path is redundant. To add scripts from another source, implement `IScriptProvider` and register it with `AddScripts(...)`; the embedded-resource helpers (`AddScriptFromEmbeddedResource` / `AddScriptsFromEmbeddedResources`) are unchanged.
