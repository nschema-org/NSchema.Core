...
- Config-in-SQL. DDL files may carry top-level configuration blocks: `NSCHEMA ( … )`, `BACKEND file ( … )`, `PROVIDER postgres ( … )` declaring orchestration metadata (dialect, state backend, live provider) alongside the schema, in SQL-statement form (mirroring Postgres `WITH (option = value, …)`). The core captures them into a generic `ConfigBlock` / `ConfigValue` model (`NSchema.Configuration`) but never interprets them: `DdlConfigReader.Read(...)` surfaces them for a front-end, while the schema path (`DdlSchemaSerializer.Read`) ignores them. Interpretation — precedence, mapping a block to builder registration, provider dispatch, secrets-from-env — is a front-end concern.
