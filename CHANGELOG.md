...
- `INamedSchemaObject` (`Name`/`OldName`/`Comment`), implemented by every named schema-object model record, and `ISchemaObjectDiff` (`Schema`/`Name`/`Kind`/`RenamedFrom`), implemented by every per-kind diff record with `SchemaDiff.EnumerateObjects()` enumerating all changed objects across kinds. These back a shared comparer skeleton — matching, rename detection, and partial-schema drop semantics now live in one place instead of per kind — and kind-agnostic diff consumers (change summary, destructive-change detection). No behavioral change.
