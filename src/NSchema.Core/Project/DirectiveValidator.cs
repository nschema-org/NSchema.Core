...
internal static class DirectiveValidator
{
    public static IEnumerable<Diagnostic> Validate(ProjectDefinition project)
    {
        ...
        foreach (var rename in directives.Schemas.Renames)
        {
            ...
            if (directives.Schemas.Drops.Contains(rename.From))
            {
        ...
        {
            if (index.FindSchema(drop) is not null)
            {
                yield return ProjectDiagnostics.DropOfDeclared("schema", drop.Value);
            }
        }

        foreach (var partial in directives.Schemas.Partials)
        {
            if (index.FindSchema(partial) is null)
            {
                yield return ProjectDiagnostics.DirectiveSchemaNotDeclared($"PARTIAL SCHEMA {partial}", partial);
            }
        }
        ...
        var kinds = new (string Kind, IReadOnlyList<ObjectRenameDirective> Renames, IReadOnlyList<ObjectReference> Drops, Func<ObjectReference, bool> IsDeclared)[]
        {
            ...
            ("enum", directives.Enums.Renames, directives.Enums.Drops, index.HasEnum),
            ("sequence", directives.Sequences.Renames, directives.Sequences.Drops, index.HasSequence),
            ("routine", directives.Routines.Renames, directives.Routines.Drops, index.HasRoutine),
            ("domain", directives.Domains.Renames, directives.Domains.Drops, index.HasDomain),
            ("composite type", directives.CompositeTypes.Renames, directives.CompositeTypes.Drops, index.HasCompositeType),
        };

        foreach (var (kind, renames, drops, isDeclared) in kinds)
        {
            foreach (var d in ValidateRenameShape(kind,
            ...
            foreach (var rename in renames)
            {
                ...
                {
                    yield return ProjectDiagnostics.DirectiveSchemaNotDeclared($"RENAME of {kind} '{rename.From}'", rename.From.Schema);
                }
                else if (!isDeclared(new ObjectReference(declaredSchema, rename.To)))
                {
                    yield return ProjectDiagnostics.RenameTargetNotDeclared(kind, rename.From.ToString(), rename.To);
                }
                if (isDeclared(new ObjectReference(declaredSchema, rename.From.Name)))
                {
                    yield return ProjectDiagnostics.RenameSourceStillDeclared(kind, rename.From.ToString(), rename.To);
                }
                if (drops.Contains(rename.From))
                {
                    yield return ProjectDiagnostics.RenameOfDropped(kind, rename.From.ToString());
                }
            }

            foreach (var drop in drops)
            {
                var declaredSchema = DeclaredSchema(drop.Schema);
                if (index.FindSchema(declaredSchema) is null)
                {
                    yield return ProjectDiagnostics.DirectiveSchemaNotDeclared($"DROP of {kind} '{drop}'", drop.Schema);
                }
                else if (isDeclared(new ObjectReference(declaredSchema, drop.Name)))
                {
                    yield return ProjectDiagnostics.DropOfDeclared(kind, drop.ToString());
                }
            }
        }
        ...
        foreach (var d in ValidateRenameShape("column",
            directives.Tables.ColumnRenames.Select(r => (Container: (r.From.Schema, r.From.Object), From: r.From.Member, r.To)).ToList(),
