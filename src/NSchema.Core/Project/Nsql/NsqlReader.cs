...
/// </summary>
public sealed class NsqlReader
{
    /// <summary>
    /// The singleton instance of <see cref="NsqlReader"/> for convenience.
    /// </summary>
    public static readonly NsqlReader Instance = new();
    ...
    public Result<NsqlDocument, NsqlDiagnostic> Read(string source)
    {
    ...
    public async Task<Result<NsqlDocument, NsqlDiagnostic>> ReadFile(string path, CancellationToken cancellationToken = default)
    {
        string source;
        try
        {
        ...
        return Result<NsqlDocument, NsqlDiagnostic>.From(
            ...
            result.Diagnostics.Select(d => d with { File = path }).ToList());
    }
}
