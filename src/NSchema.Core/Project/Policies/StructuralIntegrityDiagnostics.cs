...
internal static class StructuralIntegrityDiagnostics
{
    ...
    public static Diagnostic DuplicateObjectName(ObjectAddress name, string kind) =>
        Diagnostic.Error(Source, $"Schema '{name.Schema}' declares {kind:text} '{name.Name}' more than once.");
    ...
    public static Diagnostic CollidingObjectName(ObjectAddress name) =>
        Diagnostic.Error(Source, $"Schema '{name.Schema}' reuses the name '{name.Name}'.");
    ...
    public static Diagnostic DuplicateRoutineName(ObjectAddress routine) =>
        Diagnostic.Error(Source, $"Schema '{routine.Schema}' declares routine '{routine.Name}' more than once.");

    /// <summary>
    /// A table declared with no columns.
    /// </summary>
    public static Diagnostic EmptyTable(ObjectAddress table) =>
        Diagnostic.Error(Source, $"Table '{table}' has no columns.");

    /// <summary>
    /// A column declared more than once on a table.
    /// </summary>
    public static Diagnostic DuplicateColumn(MemberAddress column) =>
        Diagnostic.Error(Source, $"Table '{column.Owner}' declares column '{column.Member}' more than once.");
    ...
    public static Diagnostic DefaultOnGeneratedColumn(MemberAddress column) =>
        Diagnostic.Error(Source, $"Column '{column}' has both a DEFAULT and a GENERATED expression; a generated column cannot have a default.");
    ...
    public static Diagnostic UnknownPrimaryKeyColumn(MemberAddress primaryKey, SqlIdentifier column) =>
        Diagnostic.Error(Source, $"Primary key '{primaryKey.Member}' on '{primaryKey.Owner}' references unknown column '{column}'.");
    ...
    public static Diagnostic UnknownIndexColumn(MemberAddress index, SqlIdentifier column) =>
        Diagnostic.Error(Source, $"Index '{index.Member}' on '{index.Owner}' references unknown column '{column}'.");
    ...
    public static Diagnostic UnknownLocalColumn(MemberAddress foreignKey, SqlIdentifier column) =>
        Diagnostic.Error(Source, $"Foreign key '{foreignKey.Member}' on '{foreignKey.Owner}' references unknown local column '{column}'.");
    ...
    public static Diagnostic ForeignKeyArityMismatch(MemberAddress foreignKey, int local, int referenced) =>
        Diagnostic.Error(Source, $"Foreign key '{foreignKey.Member}' on '{foreignKey.Owner}' has {local} local column(s) but {referenced} referenced column(s).");
    ...
    public static Diagnostic UndeclaredForeignKeyTarget(MemberAddress foreignKey, ObjectAddress target) =>
        Diagnostic.Warning(Source, $"Foreign key '{foreignKey.Member}' on '{foreignKey.Owner}' references table '{target}', which this project does not declare; it must already exist in the database.");
    ...
    public static Diagnostic UnknownReferencedColumn(MemberAddress foreignKey, MemberAddress referenced) =>
        Diagnostic.Error(Source, $"Foreign key '{foreignKey.Member}' on '{foreignKey.Owner}' references unknown column '{referenced.Member}' on '{referenced.Owner}'.");
    ...
    public static Diagnostic ForeignKeyTargetNotUnique(MemberAddress foreignKey, ObjectAddress target, IEnumerable<SqlIdentifier> columns) =>
        Diagnostic.Error(Source, $"Foreign key '{foreignKey.Member}' on '{foreignKey.Owner}' references columns ({string.Join(", ", columns)}) on '{target}' that are not the primary key or a unique index.");
}
