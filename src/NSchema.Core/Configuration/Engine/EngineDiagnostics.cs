...
{
    internal static readonly DiagnosticSource Source = DiagnosticSources.Config;
    ...
    public static NsqlDiagnostic EngineRequirementUnsatisfied(VersionRange range, SemanticVersion version) =>
        new(Source, "engine-requirement-unsatisfied", $"This project requires an engine version matching '{range}', but this engine is {version}. Update the engine, or relax the ENGINE assertion.", DiagnosticSeverity.Error, SourcePosition.None);
    ...
    public static NsqlDiagnostic HostRequirementUnsatisfied(VersionRange range, SemanticVersion version) =>
        new(Source, "host-requirement-unsatisfied", $"This project requires a host version matching '{range}', but this host is {version}. Update the tool, or relax the ENGINE assertion.", DiagnosticSeverity.Error, SourcePosition.None);
}
