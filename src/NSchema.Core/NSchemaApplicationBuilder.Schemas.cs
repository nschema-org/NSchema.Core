...
{
    /// <summary>
    /// Adds the desired schema from the SQL files matching <paramref name="globPattern"/> under <paramref name="baseDirectory"/>.
    /// </summary>
    ...
    public NSchemaApplicationBuilder AddDdlSchemas(string baseDirectory, string globPattern = "**/*.sql")
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(globPattern);
        var matcher = new Matcher();
        matcher.AddInclude(globPattern);
        return AddDdlSchemas(baseDirectory, matcher);
    }

    /// <summary>
    /// Adds the desired schema from the SQL files the given <see cref="Matcher"/> selects under <paramref name="baseDirectory"/>.
    ...
    /// <returns>The application builder, for chaining.</returns>
    public NSchemaApplicationBuilder AddDdlSchemas(string baseDirectory, Matcher matcher)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(baseDirectory);
        ArgumentNullException.ThrowIfNull(matcher);

        Services.AddSingleton(new DdlSchemaSource(baseDirectory, matcher));
        return this;
    }
