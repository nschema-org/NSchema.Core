...
public abstract partial class SqlDialect
{
    ...
    protected virtual Result<IReadOnlyList<SqlStatement>> AddCheckConstraint(AddCheckConstraint action) =>
        Statement($"ALTER TABLE {Qualify(action.SchemaName, action.TableName)} ADD CONSTRAINT {Quote(action.CheckConstraint.Name)} CHECK ({action.CheckConstraint.Expression})");
    ...
    protected virtual Result<IReadOnlyList<SqlStatement>> DropCheckConstraint(DropCheckConstraint action) =>
        Statement($"ALTER TABLE {Qualify(action.SchemaName, action.TableName)} DROP CONSTRAINT {Quote(action.ConstraintName)}");
    ...
    protected virtual Result<IReadOnlyList<SqlStatement>> AddUniqueConstraint(AddUniqueConstraint action) =>
        Statement($"ALTER TABLE {Qualify(action.SchemaName, action.TableName)} ADD CONSTRAINT {Quote(action.UniqueConstraint.Name)} UNIQUE ({ColumnList(action.UniqueConstraint.ColumnNames)})");
