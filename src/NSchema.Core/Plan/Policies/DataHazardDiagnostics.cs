...
internal static class DataHazardDiagnostics
{
    ...
    public static Diagnostic RequiredColumnWithoutDefault(MemberAddress column, DiagnosticSeverity severity) =>
        new(Source, "required-column-without-default", $"Column '{column}' is added NOT NULL without a default value. The migration will fail if the table holds rows.", severity);
    ...
    public static Diagnostic ColumnBecomesRequired(MemberAddress column, DiagnosticSeverity severity) =>
        new(Source, "column-becomes-required", $"Column '{column}' becomes NOT NULL. The migration will fail if any existing rows are NULL.", severity);
    ...
    public static Diagnostic RiskyTypeChange(MemberAddress column, SqlType from, SqlType to, DiagnosticSeverity severity) =>
        new(Source, "risky-type-change", $"Column '{column}' changes type from {from} to {to}. The cast will fail if any existing values cannot be converted.", severity);
    ...
    public static Diagnostic PrimaryKeyOverExistingData(MemberAddress primaryKey, IReadOnlyList<SqlIdentifier> columns, DiagnosticSeverity severity) =>
        new(Source, "primary-key-over-existing-data", $"Primary key '{primaryKey.Member}' on '{primaryKey.Owner}' is added over existing {Columns(columns)}. The migration will fail if any rows hold duplicate or NULL values.", severity);
    ...
    public static Diagnostic UniqueConstraintOverExistingData(MemberAddress constraint, IReadOnlyList<SqlIdentifier> columns, DiagnosticSeverity severity) =>
        new(Source, "unique-constraint-over-existing-data", $"Unique constraint '{constraint.Member}' on '{constraint.Owner}' is added over existing {Columns(columns)}. The migration will fail if existing rows hold duplicate values.", severity);
    ...
    public static Diagnostic UniqueIndexOverExistingData(MemberAddress index, DiagnosticSeverity severity) =>
        new(Source, "unique-index-over-existing-data", $"Unique index '{index.Member}' on '{index.Owner}' is added over existing data. The migration will fail if existing rows hold duplicate values.", severity);
