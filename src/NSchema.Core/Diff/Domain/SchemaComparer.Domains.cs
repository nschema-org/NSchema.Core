...
{
    private List<DomainDiff> CompareDomains(string schemaName, IReadOnlyList<DomainDefinition> current, SchemaDefinition desired) =>
    ...
    private static DomainDiff BuildNewDomain(string schema, DomainDefinition domain) =>
    ...
    private DomainDiff? BuildModifiedDomain(string schema, DomainDefinition current, DomainDefinition desired)
    {
        ...
        IReadOnlyList<CheckConstraintDiff> checks = requiresRecreate
            ? []
            : CompareTableMembers(schema, desired.Name, "DomainDefinition check", current.Checks, desired.Checks,
