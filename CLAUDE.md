...
**Desired-state** schema comes exclusively from SQL DDL files. `AddDdlSchemas(baseDirectory, glob|Matcher)` registers a `DdlSchemaSource`; the internal `IDesiredSchemaProvider` (`DesiredSchemaProvider`) globs every registered source, reads matched files with `DdlReader`, and aggregates into a `DesiredProject` — the desired `DatabaseSchema` **plus** the scripts declared inline. **The unified `SCRIPT` statement** (`SCRIPT '<name>' RUN [ALWAYS | ONCE] ON <event> AS $$…$$;`) is the canonical form of both script kinds: `ON PRE|POST DEPLOYMENT` produces a deployment bookend (`Script`), `ON ADD COLUMN|ALTER COLUMN TYPE|ADD CONSTRAINT <path>` produces a data migration (`DataMigration`); the `RUN` clause parses to `RunCondition` on either (`UNLESS EXISTS` is reserved and rejected). **`RUN ONCE` is ledger-enforced, and planning is symmetric:** `MigrationPlanner.Plan(CurrentState current, DesiredProject desired)` compares what-I-have to what-I-want, where `CurrentState` (internal, `Plan/`) is the current schema **plus** the recorded executions as `ScriptHash`es — so the planner plans schema changes *and* script runs in one place. An already-executed run-once declaration is not part of the current→desired difference, so the planner drops it (Info diagnostic, source `run-once`; a changed body warns and stays skipped — rename to re-run) and a skipped migration neither splices, decomposes, nor suppresses hazards. `PlannedMigration.Scripts` lists the pending declarations the plan will execute (matched-only for migrations). The state envelope carries `ExecutedScripts` (internal `ScriptExecutionRecord`: name + body hash + timestamp; envelope stays version 1, old payloads read as an empty ledger); `MigrationWorkflow` translates records → `ScriptHash` at the boundary, so State models never enter the Plan domain. **The `SqlPlan` is the complete executable artifact**: it carries `Scripts` (public `ScriptHash` in `NSchema.Sql.Model`: name + body hash) alongside its statements — `PlanOperation` enriches the generator's output (`Generate(plan) with { Scripts = … }`), so providers never know about the manifest and everything that moves the SQL (plan result, plan file, apply arguments) moves the manifest with it; there are no parallel run-once parameters anywhere. A successful apply passes the applied plan to `IMigrationWorkflow.Refresh(applied, ct)`, which records its manifest into the carried-over ledger (refresh never rebuilds the ledger and treats a corrupt existing payload as empty — it stays the recovery path); a failed or empty apply passes nothing and records nothing. Planning with run-once scripts but no store warns. The internal `IScriptDeclaration` capability interface (`Name?`/`Sql`/`RunCondition`, implemented by both `Script` and `DataMigration` — ABI-invisible) is what machinery treating the two kinds identically codes against (run-once resolution, execution hashing, name validation); the model merge itself waits for 5.0 because the planner's edges (parser output, `MigrationPlan` script lists in the `ISqlGenerator` contract, diff annotations, `ExecuteDataMigration`) are public. The pre-4.5 spellings (`PRE|POST DEPLOYMENT '<name>' AS $$…$$;`, `MIGRATION ['name'] FOR <trigger> <path> AS $$…$$;`) still parse into the same models but emit a `DdlWarning` on the parser's `DdlParseResult` (warnings describe the read, so no model carries them; `DdlReader.Read` keeps its document-only signature). `DesiredSchemaProvider` converts them to file-stamped `deprecations` diagnostics on `DesiredProjectResult` — `GetProject` returns that result record (project + `Files` + `Diagnostics`), the pure planner receives only the `DesiredProject` payload, and `MigrationWorkflow` merges the read diagnostics into plan/validate outcomes — removal in 5.0. Script names are validated unique project-wide after template expansion. `GetProject(scope, ct)` is its one method; there is no in-code desired-schema seam and no `ISchemaTransformer`. Multiple `AddDdlSchemas` calls aggregate (e.g. a base set plus an environment overlay).

The DDL stack lives in `Schema/Ddl/`: `DdlLexer` → `DdlParser` (partials per statement family: `.Create`, `.Drop`, `.Grant`, `.Config`, `.Scripts`, `.Templates`, `.SchemaAccumulator`) → schema model. **Schema templates** (`TEMPLATE name BEGIN … END;` + `APPLY TEMPLATE name IN SCHEMA a, b;`) declare a reusable object group instantiated per schema: bodies are fully parsed (no views/schemas/extensions/drops inside; unqualified names bind to the target schema, qualified ones escape), and the internal `TemplateExpander` expands applications in `DesiredSchemaProvider` after all sources aggregate — location- and order-agnostic, invisible to diff/plan/import. `SCRIPT`s of either event kind are allowed in template bodies — change-event scripts use an unqualified `table.member` path and must target a table the template declares; deployment-bookend scripts instantiate one `Script` per applied schema, and a scoped read drops out-of-scope instances by their origin schema (tracked internally via `TemplateScriptInstance`, never on the `Script` model). The `{schema}` token substitutes in both the SQL and the script name (names must end up unique per instance). **Table templates** (`TEMPLATE name FOR TABLE BEGIN <members> END;`) hold comma-separated table members instead of statements and merge into a table via an `INCLUDE name` member (columns land at the include position); includes resolve after schema-template application, so a templated table can itself include one. Duplicate index-backed names per schema (a hazard when a mixin carries an index) are caught by `StructuralIntegritySchemaPolicy`, templates or not. `DdlWriter` renders the model back to DDL (used by Import). `DdlFormatter` is a gentle token-stream reformatter (backs the CLI `fmt` command) — it pretty-prints without going through the full parse/model round-trip, so it preserves what it doesn't understand.
