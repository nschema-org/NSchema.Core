...
- **`ForceUnlockOperation`** — calls `IStateLock.ForceUnlock` to remove a stale state lock regardless of holder, and reports who held it. The analogue of `terraform force-unlock`. Does not plan, apply, or touch the schema. No-op (reports "no lock held") under the default `NoOpStateLock`.
