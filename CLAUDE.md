...
**Desired-state** schema comes exclusively from SQL DDL files. `AddDdlSchemas(baseDirectory, glob|Matcher)` registers a `DdlSchemaSource`; the internal `IDesiredSchemaProvider` (`DesiredSchemaProvider`) globs every registered source, reads matched files with `DdlReader`, and aggregates into a `DesiredProject` — the desired `DatabaseSchema` **plus** the scripts declared inline. **The unified `SCRIPT` statement** (`SCRIPT '<name>' RUN [ALWAYS | ONCE] ON <event> AS $$…$$;`) is the canonical form of both script kinds: `ON PRE|POST DEPLOYMENT` produces a deployment bookend (`Script`), `ON ADD COLUMN|ALTER COLUMN TYPE|ADD CONSTRAINT <path>` produces a data migration (`DataMigration`); the `RUN` clause parses to `RunCondition` on either (`UNLESS EXISTS` is reserved and rejected). **`RUN ONCE` is ledger-enforced:** the state envelope carries `ExecutedScripts` (internal `ScriptExecutionRecord`: name + body hash + timestamp; envelope stays version 1, old payloads read as an empty ledger); the workflow reads the ledger and passes it to `MigrationPlanner.Plan`, which drops recorded run-once declarations (Info diagnostic, source `run-once`; a changed body warns and stays skipped — rename to re-run) before matching, so a skipped migration neither splices, decomposes, nor suppresses hazards. Pending run-once scripts ride out as public `RunOnceScript` (name + hash) on `PlannedMigration` → `PlanResult.RunOnceScripts` → `PlanFileEnvelope.RunOnceScripts` → `ApplyArguments.RunOnceScripts`; a successful apply passes them to `IMigrationWorkflow.Refresh(executedScripts, ct)`, which merges them into the carried-over ledger (refresh never rebuilds the ledger and treats a corrupt existing payload as empty — it stays the recovery path). A failed apply records nothing. Planning with run-once scripts but no store warns. The pre-4.5 spellings (`PRE|POST DEPLOYMENT '<name>' AS $$…$$;`, `MIGRATION ['name'] FOR <trigger> <path> AS $$…$$;`) still parse into the same models but emit a `DdlWarning` on the parser's `DdlParseResult` (warnings describe the read, so no model carries them; `DdlReader.Read` keeps its document-only signature). `DesiredSchemaProvider` converts them to file-stamped `deprecations` diagnostics on `DesiredProjectResult` — `GetProject` returns that result record (project + `Files` + `Diagnostics`), the pure planner receives only the `DesiredProject` payload, and `MigrationWorkflow` merges the read diagnostics into plan/validate outcomes — removal in 5.0. Script names are validated unique project-wide after template expansion. `GetProject(scope, ct)` is its one method; there is no in-code desired-schema seam and no `ISchemaTransformer`. Multiple `AddDdlSchemas` calls aggregate (e.g. a base set plus an environment overlay).
